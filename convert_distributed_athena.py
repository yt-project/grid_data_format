...
def write_gdf_field(fn, grid_number, field, data):
    f = h5.File(fn,'a')
    ## --------- Store Grid Data --------- ##
...
def read_and_write_hierarchy(basename, ddn, gdf_name):
    ...
    pars_g.attrs['current_time'] = current_time
...
def read_and_write_data(basename, ddn, gdf_name):
    proc_names = glob('id*')
    print 'Reading a dataset from %i Processor Files' % len(proc_names)
    N = len(proc_names)
    fields=[]
    for i in range(N):
        ...
        while line is not '':
            ...
            if 'SCALARS' in splitup:
                ...
                write_gdf_field(gdf_name, i, field, data)
                read_table=False
                ...
                if i == 0:
                    fields.append(field+'_x')
                    fields.append(field+'_y')
                    fields.append(field+'_z')

                print 'writing field %s' % field
                write_gdf_field(gdf_name, i, field+'_x', data_x)
                write_gdf_field(gdf_name, i, field+'_y', data_y)
                write_gdf_field(gdf_name, i, field+'_z', data_z)
...
n=30
basename = 'kh_3d_mhd_hlld_128_beta5000_sub_tanhd'
fn = basename+'.%04i'%n
read_and_write_hierarchy(basename, n ,fn+'.h5')
read_and_write_data(basename, n ,fn+'.h5')
